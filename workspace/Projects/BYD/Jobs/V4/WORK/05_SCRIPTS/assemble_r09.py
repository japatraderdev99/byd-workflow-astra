...
b=ROOT/'Projects/BYD/Jobs/V4';spec=json.loads((b/'WORK/00_MATRIZ/production_spec_r03.json').read_text());r7=json.loads((b/'WORK/00_MATRIZ/production_spec_r07.json').read_text());spec['offers']=r7['offers'];spec.update(revision='r09',output_templates='WORK/01_TEMPLATES/2026-09-20-r09/',output_staging='WORK/03_STAGING/2026-09-20-r09/')
plan=[]
...
(b/'WORK/00_MATRIZ/production_spec_r09.json').write_text(json.dumps(spec,ensure_ascii=True,indent=2));(b/'WORK/04_QA/assembly_r09.json').write_text(json.dumps({'at':datetime.datetime.now().astimezone().isoformat(),'method':'byte copies; no image recompression or PSD mutation','files':rows},indent=2));print('OK|ASSEMBLED|'+str(len(rows)))

...
(function(){
var ROOT=MK.root(),BASE=ROOT+'/Projects/BYD/Jobs/V4/',REFUSE_OVERWRITE=true,src=null,doc=null;
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p01.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p01.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
function dup(id,parent,name){app.activeDocument=src;var l=find(src,id);if(!l)throw Error('MISSING '+id);var cp=l.duplicate(doc,ElementPlacement.PLACEATBEGINNING);app.activeDocument=doc;if(parent)cp.move(parent,ElementPlacement.INSIDE);if(name)cp.name=name;cp.visible=true;return cp;}
...
try{
 ...
 var desc=new ActionDescriptor();desc.putPath(charIDToTypeID('null'),new File(BASE+spec.logo));desc.putEnumerated(charIDToTypeID('FTcs'),charIDToTypeID('QCSt'),charIDToTypeID('Qcsa'));executeAction(charIDToTypeID('Plc '),desc,DialogModes.NO);var logo=doc.activeLayer;logo.name='LOGO';logo.move(fix,ElementPlacement.INSIDE);fit(logo,z.logo,'center');
 var states=[];
 ...
 for(var oi=0;oi<states.length;oi++){for(var j=0;j<states.length;j++)for(var k=0;k<states[j].length;k++)states[j][k].visible=(j===oi);savepng('p01_byd_'+spec.offers[oi].id+'_'+fmt.id);}
 var psdf=new File(BASE+'WORK/01_TEMPLATES/P01_TRIAL_'+fmt.id+'.psd');if(psdf.exists)throw Error('REFUSE_OVERWRITE');var po=new PhotoshopSaveOptions();po.layers=true;po.embedColorProfile=true;doc.saveAs(psdf,po,true,Extension.LOWERCASE);
 msg('OK|template_trial|'+fmt.id+'|elapsed_ms='+((new Date().getTime())-t0));doc.close(SaveOptions.DONOTSAVECHANGES);doc=null;
 }
 msg('OK|completed');
}catch(e){msg('ERRO|'+e.message+'|line='+e.line);}finally{if(doc)doc.close(SaveOptions.DONOTSAVECHANGES);if(src)src.close(SaveOptions.DONOTSAVECHANGES);log.close();}
})();

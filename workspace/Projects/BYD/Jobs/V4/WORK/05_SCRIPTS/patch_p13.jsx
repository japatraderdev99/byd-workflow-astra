...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p13.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/patch_p13.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.encoding='UTF8';log.open('w');
...
var out=new File(BASE+spec.output_templates+'P13_TRIAL_'+fmt.id+'.psd');if(out.exists)throw Error('REFUSE_OVERWRITE');
...
var dark=byName(bg.layerSets[bi],'CONTRASTE · KV MULTIPLICADO');if(!dark)throw Error('MISSING_CONTRAST');
for(var di=0;di<dark.layers.length;di++)dark.layers[di].visible=false;
var shade=dark.artLayers.add();shade.name='AJUSTE CONTRASTE · azul profundo · opacidade editavel';doc.activeLayer=shade;
var color=new SolidColor();color.rgb.hexValue='061D34';doc.selection.selectAll();doc.selection.fill(color,ColorBlendMode.NORMAL,100,false);doc.selection.deselect();shade.opacity=78;dark.name='CONTRASTE · mascara suave';
}
}
if(fmt.family==='strip'){
var cg=doc.layerSets.getByName('CONDICIONAIS');
var ex=byName(cg,'COND · instal · dolphin-mini-5l-gs');if(ex){ex.textItem.contents=wrapText(ex.textItem.contents,15);fit(ex,fmt.zones.instal,'center');}
var dt=byName(cg,'ENQUADRAMENTO · detail · dolphin-mini-5l-gs');if(dt)dt.visible=false;msg('OMISSAO_PROPOSTA|'+fmt.id+'|detail Dolphin: insignia pequena demais; preservada oculta');
}
...
var pf=new File(BASE+spec.output_pilots+'p13_byd_'+id+'_'+fmt.id+'.png');if(pf.exists)throw Error('REFUSE_OVERWRITE');var eo=new ExportOptionsSaveForWeb();eo.format=SaveDocumentType.PNG;eo.PNG8=false;eo.transparency=false;eo.includeProfile=true;doc.exportDocument(pf,ExportType.SAVEFORWEB,eo);msg('OK|png|'+id+'|'+fmt.id);
}

#!/usr/bin/env python3
"""Read-only structural QA for seven production template PSDs.

R02 fixes the R01 state-name parser. Production names under CONDICIONAIS and
VEICULO are ``COND - offer-id`` and ``CAR - offer-id`` after canonicalization;
R01 compared those full names directly with ``offer-id`` and therefore
reported zero states, missing text, and missing anchors for every template.
...
def state_groups(container: Any, offer_ids: set[str]) -> dict[str, list[Any]]:
    ...
    for child in direct_child_groups(container):
        ...
        state_id = key if key in offer_ids else None
        if state_id is None:
            for prefix in ("cond-", "car-"):
                if key.startswith(prefix) and key[len(prefix):] in offer_ids:
                    state_id = key[len(prefix):]
                    break
        if state_id is not None:
            found[state_id].append(child)
    return found
...
def main() -> int:
    ...
    default_output = V4 / f"WORK/04_QA/qa_templates_{args.revision}_v2.json"
    ...
    print(f"OK|qa_templates_r02|templates={len(reports)}|report={output.relative_to(ROOT)}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERRO|qa_templates_r02|{type(exc).__name__}|{exc}", file=sys.stderr)
        raise

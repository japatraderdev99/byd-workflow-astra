...
b=Path(__file__).resolve().parents[2];w=b/'WORK';d=json.loads((w/'00_MATRIZ/pilot_spec_p16.json').read_text());out=w/'02_PILOTOS/2026-09-19-p16';sheet=out/'PRANCHA_21_PILOTOS_P16.jpg';page=out/'REVISAO_P16.html'
if sheet.exists() or page.exists():raise RuntimeError('REFUSE_OVERWRITE')
files=list(out.glob('p16_byd_*.png'))
if len(files)!=21:raise RuntimeError('EXPECTED_21_PNG')
font=ImageFont.load_default(size=20)
canvas=Image.new('RGB',(1800,3300),'#101820');draw=ImageDraw.Draw(canvas);draw.text((30,20),'BYD V4 / P16 — 21 PILOTOS · AGUARDANDO REVISAO HUMANA',font=font,fill='white')
blocks=[]
...
for row,f in enumerate(d['formats']):
 ...
 for col,o in enumerate(d['offers']):
  name='p16_byd_'+o['id']+'_'+f['id']+'.png';p=out/name

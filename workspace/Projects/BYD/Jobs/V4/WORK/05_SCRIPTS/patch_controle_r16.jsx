...
/*
  R16 cold post-production patch for confirmed BYD review findings.
  ...
  1. In the two confirmed landscape formats, tighten only the bottom of the
     ...
     R16 keeps the full car zone (100%) and feathers the new bottom edge by at
  ...
  R16 runtime correction: activate the target state before every visibility
  read or mutation. R12 stopped before creating output because its strip state
  was inactive. R12 and its failure log remain immutable evidence.
...
var LOG_REL = 'WORK/06_LOGS/patch_controle_r16.log';
var OUT_TPL_REL = 'WORK/01_TEMPLATES/2026-09-20-r16/';
var OUT_PNG_REL = 'WORK/03_STAGING/2026-09-20-r16/';
...
function verifyComps(label) { if (doc.layerComps.length !== 20) throw Error('LAYER_COMP_COUNT|' + label + '|' + doc.layerComps.length); msg('LAYER_COMPS_OK|' + label + '|20'); }
...
function patchHeadlineIfExcessive(fmt, headlineZone) {
 ...
 headline.textItem.useAutoLeading = true;
 headline.textItem.autoLeadingAmount = 110;
 app.refresh();
 headline = directLayer(state, 'COND \u00b7 HEADLINE \u00b7 ' + DOLPHIN_ID);
 if (headline.textItem.contents !== contents) throw Error('HEADLINE_CONTENT_CHANGED|' + fmt.id);
 var tightened = box(headline);
 msg('LEADING_DIAG|' + fmt.id + '|auto=' + headline.textItem.useAutoLeading + '|autoAmount=' + headline.textItem.autoLeadingAmount + '|before=' + before.join(',') + '|after_auto=' + tightened.join(','));
 var fitted = fitTop(headline, headlineZone, fmt.align);
 headline = directLayer(state, 'COND \u00b7 HEADLINE \u00b7 ' + DOLPHIN_ID);
 if (!headline.textItem.useAutoLeading || Math.abs(headline.textItem.autoLeadingAmount - 110) > .01) throw Error('AUTO_LEADING_NOT_APPLIED|' + fmt.id);
...
function preflightOutputs() {
 ...
 if (!outDir.exists || !pngDir.exists) throw Error('MISSING_R16_OUTPUT_FOLDER');
...
try {
 ...
 for (var fi = 0; fi < formats.length; fi++) {
  ...
  if (cfg.checkHeadline || cfg.stripInstallment) {
   activate(DOLPHIN_ID);
   if (cfg.checkHeadline) headlineChanged = patchHeadlineIfExcessive(fmt, fmt.zones.headline);
  ...
  if (cfg.applyMask) { activate(VD_ID); tightenVehicleMask(fmt, fmt.zones.car); }
  if (cfg.applyYuan) { activate(YUAN_ID); patchYuanScene(fmt, fmt.zones.car); }
  ...
  verifyComps('before_save|' + cfg.id);
  ...
  if (cfg.applyMask) { activate(VD_ID); exportPNG(new File(BASE + OUT_PNG_REL + 'byd_' + VD_ID + '_' + cfg.id + '.png'), cfg.id + '|' + VD_ID); verifyComps('after_export|' + cfg.id + '|' + VD_ID); pngCount++; }
  if (cfg.applyYuan) { activate(YUAN_ID); exportPNG(new File(BASE + OUT_PNG_REL + 'byd_' + YUAN_ID + '_' + cfg.id + '.png'), cfg.id + '|' + YUAN_ID); verifyComps('after_export|' + cfg.id + '|' + YUAN_ID); pngCount++; }
  if (headlineChanged || cfg.stripInstallment) { activate(DOLPHIN_ID); exportPNG(new File(BASE + OUT_PNG_REL + 'byd_' + DOLPHIN_ID + '_' + cfg.id + '.png'), cfg.id + '|' + DOLPHIN_ID); verifyComps('after_export|' + cfg.id + '|' + DOLPHIN_ID); pngCount++; }

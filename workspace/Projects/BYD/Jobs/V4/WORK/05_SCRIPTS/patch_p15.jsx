...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p15.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/patch_p15.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.encoding='UTF8';log.open('w');
...
var out=new File(BASE+spec.output_templates+'P15_TRIAL_'+fmt.id+'.psd');if(out.exists)throw Error('REFUSE_OVERWRITE');
doc=app.open(new File(BASE+'WORK/01_TEMPLATES/2026-09-19-p12/P12_TRIAL_'+fmt.id+'.psd'));
var visibility=[];function repairNames(g){for(var ri=0;ri<g.layers.length;ri++){var rl=g.layers[ri];visibility.push([rl,rl.visible]);if(rl.typename==='LayerSet')repairNames(rl);}}repairNames(doc);for(var vi=0;vi<visibility.length;vi++){var vl=visibility[vi][0],nn=vl.name.split('\u00ac\u2211').join('\u00b7');if(nn!==vl.name)vl.name=nn;}for(var vi=visibility.length-1;vi>=0;vi--)visibility[vi][0].visible=visibility[vi][1];msg('OK|visibility_restored|'+fmt.id+'|layers='+visibility.length);if(fmt.contrast_edge){
...
var pf=new File(BASE+spec.output_pilots+'p15_byd_'+id+'_'+fmt.id+'.png');if(pf.exists)throw Error('REFUSE_OVERWRITE');var eo=new ExportOptionsSaveForWeb();eo.format=SaveDocumentType.PNG;eo.PNG8=false;eo.transparency=false;eo.includeProfile=true;doc.exportDocument(pf,ExportType.SAVEFORWEB,eo);msg('OK|png|'+id+'|'+fmt.id);
}

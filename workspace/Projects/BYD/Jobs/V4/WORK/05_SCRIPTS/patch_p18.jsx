...
var log=new File(BASE+'WORK/06_LOGS/patch_p18.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.encoding='UTF8';log.open('w');function msg(s){log.writeln(new Date().toUTCString()+'|'+s);log.close();log.open('a');}
...
var out=new File(BASE+'WORK/01_TEMPLATES/2026-09-19-p18/P18_TRIAL_360x80.psd');if(out.exists)throw Error('REFUSE_OVERWRITE');
doc=app.open(new File(BASE+'WORK/01_TEMPLATES/2026-09-19-p16/P16_TRIAL_360x80.psd'));
var fix=doc.layerSets.getByName('FIXO'),footer=byName(fix,'RODAPE');footer.visible=true;function unlinkTree(g){for(var u=0;u<g.layers.length;u++){var l=g.layers[u];try{l.unlink();}catch(e){}if(l.typename==='LayerSet')unlinkTree(l);}}unlinkTree(footer);
...
fit(byName(doc.layerSets.getByName('OFERTA').layerSets.getByName(id),'PRECO'),[95,19,145,25],'left');var cg=doc.layerSets.getByName('CONDICIONAIS'),be=byName(cg,'COND \u00b7 BENEFICIO \u00b7 '+id),he=byName(cg,'COND \u00b7 HEADLINE \u00b7 '+id);fit(id.indexOf('vd-')===0?he:be,[95,48,255,7],'center');be.visible=id.indexOf('vd-')!==0;he.visible=id.indexOf('vd-')===0;
...
function dump(g){for(var q=0;q<g.layers.length;q++){var v=g.layers[q];msg('GEOM|'+v.name+'|'+box(v).join(',')+'|visible='+v.visible);if(v.typename==='LayerSet')dump(v);}}dump(footer);var f=new File(BASE+'WORK/02_PILOTOS/2026-09-19-p18/p18_byd_'+id+'_360x80.png');if(f.exists)throw Error('REFUSE_OVERWRITE');var eo=new ExportOptionsSaveForWeb();eo.format=SaveDocumentType.PNG;eo.PNG8=false;eo.transparency=false;eo.includeProfile=true;doc.exportDocument(f,ExportType.SAVEFORWEB,eo);msg('OK|png|'+id);
}

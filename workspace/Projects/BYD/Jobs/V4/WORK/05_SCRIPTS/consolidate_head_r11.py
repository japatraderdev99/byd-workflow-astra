...
OUT = B / 'WORK/04_QA/production-r11/head_visual_review.json'
...
technical = json.loads((B/'WORK/04_QA/production-r11/technical_all.json').read_text())
...
OUT.write_text(json.dumps(dict(at=datetime.now(timezone.utc).isoformat(), reviewer='Astra head',
    scope='Visual composition review by agent, not client or legal approval; no external delivery.',

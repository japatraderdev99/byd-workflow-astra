...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p09.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p09.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
function sceneMask(g,z){app.activeDocument=doc;doc.activeLayer=g;var x=z[0]-50,y=-100,r=z[0]+z[2]+50,b=doc.height.as('px')+100;doc.selection.select([[x,y],[r,y],[r,b],[x,b]]);doc.selection.feather(30);var d=new ActionDescriptor(),ref=new ActionReference();d.putClass(charIDToTypeID('Nw  '),charIDToTypeID('Chnl'));ref.putEnumerated(charIDToTypeID('Chnl'),charIDToTypeID('Chnl'),charIDToTypeID('Msk '));d.putReference(charIDToTypeID('At  '),ref);d.putEnumerated(charIDToTypeID('Usng'),charIDToTypeID('UsrM'),charIDToTypeID('RvlS'));executeAction(charIDToTypeID('Mk  '),d,DialogModes.NO);doc.selection.deselect();}
function savepng(n){var f=new File(BASE+'WORK/02_PILOTOS/'+n+'.png');if(f.exists)throw Error('REFUSE_OVERWRITE');var ex=new ExportOptionsSaveForWeb();ex.format=SaveDocumentType.PNG;ex.PNG8=false;ex.transparency=false;ex.interlaced=false;ex.includeProfile=true;doc.exportDocument(f,ExportType.SAVEFORWEB,ex);msg('OK|png|'+n);}
try{
 ...
 for(var oi=0;oi<spec.offers.length;oi++){
  ...
  var bb=box(bk);if(bb[0]>0||bb[1]>0||bb[2]<fmt.w||bb[3]<fmt.h){var scale=Math.max(fmt.w/(bb[2]-bb[0]),fmt.h/(bb[3]-bb[1]));bk.resize(scale*100,scale*100,AnchorPosition.MIDDLECENTER);bb=box(bk);bk.translate(fmt.w/2-(bb[0]+bb[2])/2,-bb[1]-100);}
  ...
  var price=dup(o.price,og,'PRECO');fit(price,o.id==='vd-atto-2'?[450,67,500,102]:z.price,'left');
 ...
 for(var oi=0;oi<states.length;oi++){for(var j=0;j<states.length;j++)for(var k=0;k<states[j].length;k++)states[j][k].visible=(j===oi);savepng('p09_byd_'+spec.offers[oi].id+'_'+fmt.id);}
 var psdf=new File(BASE+'WORK/01_TEMPLATES/P09_TRIAL_'+fmt.id+'.psd');if(psdf.exists)throw Error('REFUSE_OVERWRITE');var po=new PhotoshopSaveOptions();po.layers=true;po.embedColorProfile=true;doc.saveAs(psdf,po,true,Extension.LOWERCASE);

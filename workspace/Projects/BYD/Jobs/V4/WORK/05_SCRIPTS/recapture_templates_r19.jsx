...
/*
  R19 is the cached-DOM continuation of R17 for the six formats after strip.
  ...
  reads the cached 100 state references without rescanning native collections.
*/
(function () {
...
var LOG_REL = 'WORK/06_LOGS/recapture_templates_r19.log';
...
function buildStateCache(fmt, offers) {
 ...
 var cache = { roots: roots, tops: [], states: [], current: null, comps: {} };
...
function makeComps(fmtId, offers) {
 for (var i = 0; i < offers.length; i++) {
  ...
  var id = offers[i].id, comp = doc.layerComps.add('OFERTA ' + id, 'offer_id=' + id + '|r19_visibility_only=true', false, false, true);
  ...
  stateCache.comps[id] = comp;
 }
 if (doc.layerComps.length !== offers.length) throw Error('LAYER_COMP_COUNT_AFTER_CREATE|' + fmtId + '|' + doc.layerComps.length);
}
function indexPersistedComps(fmtId, offers) {
 var byName = {}, counts = {};
 for (var ci = 0; ci < doc.layerComps.length; ci++) { var compName = doc.layerComps[ci].name; byName[compName] = doc.layerComps[ci]; counts[compName] = (counts[compName] || 0) + 1; }
 if (doc.layerComps.length !== offers.length) throw Error('PERSISTED_COMP_COUNT|' + fmtId + '|' + doc.layerComps.length);
 for (var oi = 0; oi < offers.length; oi++) { var wanted = 'OFERTA ' + offers[oi].id; if (counts[wanted] !== 1) throw Error('LAYER_COMP_NAME_COUNT|' + fmtId + '|' + wanted + '|' + (counts[wanted] || 0)); stateCache.comps[offers[oi].id] = byName[wanted]; }
}
function testAllComps(fmtId, offers, phase) {
 for (var i = 0; i < offers.length; i++) {
  var target = i, decoy = (i + 1) % offers.length, id = offers[target].id;
  activate(decoy, offers);
  var comp = stateCache.comps[id];
  if (!comp) throw Error('MISSING_CACHED_COMP|' + phase + '|' + fmtId + '|' + id);
...
try {
 ...
 msg('SCRIPT_PREPARED|recapture_templates_r19.jsx|scope=remaining_6_templates_continuing_R17_destination|r17_strip_format_ok=true|optimization=cached_DOM_references_and_previous_target_visibility_switch|action_manager=false|tests_per_format=40|png_exports=0|approved_pngs_untouched=true');
 for (var fi = 0; fi < formats.length; fi++) {
  ...
  doc = app.open(output); validateDocument(fmt, offers); indexPersistedComps(fmt.id, offers);
  initializeActive(0, offers);

...
V=ROOT/'Projects/BYD/Jobs/V4'
REFUSE_OVERWRITE=True
out=V/'RELATORIO_PROCESSO_E_KPIS_R18.md'
if out.exists():raise RuntimeError('REFUSE_OVERWRITE')
d=json.loads((V/'WORK/04_QA/process-audit-r18/metrics.json').read_text())
runs=json.loads((V/'WORK/04_QA/production_timings_r18.json').read_text())['runs']
def num(n):return f'{n:,}'.replace(',','.')
def elapsed(s):
 h=int(s//3600);m=int(s%3600//60);sec=s%60
 return f'{h:02d}h {m:02d}min {sec:06.3f}s'
def table(headers,rows):return '| '+' | '.join(headers)+' |\n| '+' | '.join(['---']*len(headers))+' |\n'+''.join('| '+' | '.join(map(str,r))+' |\n' for r in rows)
t=d['tokens']
text='''# BYD V4 — relatório completo de execução, workflow e KPIs
...
'''
text+=f'''**Início persistido:** 19/09/2026, 12:05:22.104.  
**Fechamento local:** 20/09/2026, 02:22:25.837.  
**Tempo total de calendário documentado: {elapsed(d['documented_wall_seconds'])}.**
...
'''
text+=table(['Janela cronológica','Duração de calendário','Tokens observados no intervalo'],[(p['name'],elapsed(p['wall_seconds']),num(p['usage']['total_tokens'])) for p in d['temporal_buckets']])
text+='''
...
'''
text+=f'''- **18 runs com terminal único `OK|completed`: {elapsed(d['completed_terminal_ms']/1000)}.** Essa soma inclui um teste de reuso que terminou tecnicamente, mas foi reprovado visualmente.
- **22 runs com terminais ou trechos concluídos aproveitáveis, sem somar eventos aninhados: {elapsed(d['selected_elapsed_ms']/1000)}.** Inclui, adicionalmente, P06, strip R03, strip corrigido R14 e strip reparado R17. É uma soma parcial de durações registradas, não o tempo ativo total.
- **R10: 120 PNGs + seis PSDs em 27min06,940s**; média amortizada de {d['metrics']['r10_amortized_seconds_per_output']:.2f}s por PNG, ou {d['metrics']['root_r10_outputs_per_minute']:.2f} PNGs/min. Inclui montagem/salvamento; não é latência pura de exportação.
...
'''
fr=[]
for fmt in d['counts_by_format']:
 f=d['format_elapsed_ms'][fmt]
 production=f.get('r10',f.get('r03'))
 patch=f.get('r16',f.get('r14'))
 repair=f.get('r20',f.get('r17_partial_strip'))
 fr.append([fmt,elapsed(production/1000),elapsed(patch/1000) if patch else 'Sem patch visual de escala',elapsed(repair/1000)])
text+=table(['Formato','Construção + PSD + 20 PNGs','Patch seletivo','Reparo de comps + 40 testes'],fr)
text+='''
...
'''
text+=f'''O primeiro registro de uso recuperado é de 19/09 às 12:46:59.793: há **{elapsed(d['uncovered_initial_wall_seconds'])} iniciais sem cobertura nesta coleta**. Portanto o total abaixo é o consumo comprovado do trecho auditado, e o total integral do job permanece não determinado. Eventos após 02:22:25.837 — inclusive a resposta de encerramento e este relatório — não entram.

'''
text+=table(['Métrica','Valor observado','Interpretação'],[
 ['Entrada total',num(t['input_tokens']),'Inclui cache; processamento acumulado de contexto em múltiplas respostas'],
 ['Entrada em cache',num(t['cached_input_tokens']),'Subconjunto da entrada; não somar novamente'],
 ['Entrada sem cache',num(d['uncached_input_tokens']),'Entrada total menos entrada em cache'],
 ['Saída',num(t['output_tokens']),'Total de saída registrado'],
 ['Raciocínio na saída',num(t['reasoning_output_tokens']),'Subconjunto da saída; não somar novamente'],
 ['Total',num(t['total_tokens']),'Entrada + saída, não texto único do projeto'],
 ['Proporção de entrada em cache',f"{100*d['cached_input_fraction']:.2f}%",'Não equivale a desconto financeiro medido'],
 ['Respostas com registro de uso',num(d['response_count']),'Respostas intermediárias do agente; não mensagens do usuário'],
 ['Compactações registradas',d['compactions'],'Somadas nas três tarefas'],
])
text+='\n### Distribuição por agente e modelo registrado\n\n'
text+=table(['Agente','Modelo/esforço em turn_context','Respostas','Entrada','Cache','Saída','Total'],[
 [a['role'],', '.join(a['registered_context_models']),a['usage_record_count'],num(a['tokens']['input_tokens']),num(a['tokens']['cached_input_tokens']),num(a['tokens']['output_tokens']),num(a['tokens']['total_tokens'])] for a in d['agents']])
text+='''
...
'''
text+=table(['Tipo de despacho registrado','Quantidade'],sorted(d['tool_dispatches'].items(),key=lambda x:-x[1]))
text+=f'''
Total: **{d['tool_dispatch_count']} despachos diretos registrados**, incluindo {d['tool_dispatches'].get('exec',0)} chamadas ao orquestrador `exec`. Uma chamada exec pode conter várias ferramentas; estes números não são o total de chamadas nativas do Photoshop, ações de UI ou requisições faturáveis. Os {d['tool_dispatches'].get('send_message',0)} envios entre agentes e {d['tool_dispatches'].get('followup_task',0)} follow-ups mostram que a coordenação teve peso operacional relevante. Não é possível atribuir todo token excedente a essa coordenação sem instrumentação por chamada/atividade.
...
'''
text+=table(['KPI','Fórmula / denominador','Resultado','Status / limite'],[
...
'''
text+=table(['Run','Estado registrado','Tempo terminal','Log'],[[r['run'],r['status'],elapsed(r['terminal']['elapsed_ms']/1000) if r.get('terminal') else '—',r['log']] for r in runs])
out.write_text(text)
print('OK|PROCESS_REPORT_R18|'+str(out.relative_to(ROOT)))

...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p08.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p08.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
try{
 ...
 for(var oi=0;oi<spec.offers.length;oi++){
  ...
  var vehicle=dup(o.car,vg,'CENA ORIGINAL',o.anchor_id);affine(vehicle,s,dx,dy,o.anchor_name);
 ...
 for(var oi=0;oi<states.length;oi++){for(var j=0;j<states.length;j++)for(var k=0;k<states[j].length;k++)states[j][k].visible=(j===oi);savepng('p08_byd_'+spec.offers[oi].id+'_'+fmt.id);}
 var psdf=new File(BASE+'WORK/01_TEMPLATES/P08_TRIAL_'+fmt.id+'.psd');if(psdf.exists)throw Error('REFUSE_OVERWRITE');var po=new PhotoshopSaveOptions();po.layers=true;po.embedColorProfile=true;doc.saveAs(psdf,po,true,Extension.LOWERCASE);

...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p14.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/patch_p14.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.encoding='UTF8';log.open('w');
...
var out=new File(BASE+spec.output_templates+'P14_TRIAL_'+fmt.id+'.psd');if(out.exists)throw Error('REFUSE_OVERWRITE');
doc=app.open(new File(BASE+'WORK/01_TEMPLATES/2026-09-19-p12/P12_TRIAL_'+fmt.id+'.psd'));
function repairNames(g){for(var ri=0;ri<g.layers.length;ri++){var rl=g.layers[ri];rl.name=rl.name.split('\u00ac\u2211').join('\u00b7');if(rl.typename==='LayerSet')repairNames(rl);}}repairNames(doc);if(fmt.contrast_edge){
...
var color=new SolidColor();color.rgb.hexValue='061D34';doc.selection.selectAll();doc.selection.fill(color,ColorBlendMode.NORMAL,100,false);doc.selection.deselect();shade.opacity=78;dark.name='CONTRASTE \xb7 mascara suave';
}
}
if(fmt.family==='strip'){
...
var pf=new File(BASE+spec.output_pilots+'p14_byd_'+id+'_'+fmt.id+'.png');if(pf.exists)throw Error('REFUSE_OVERWRITE');var eo=new ExportOptionsSaveForWeb();eo.format=SaveDocumentType.PNG;eo.PNG8=false;eo.transparency=false;eo.includeProfile=true;doc.exportDocument(pf,ExportType.SAVEFORWEB,eo);msg('OK|png|'+id+'|'+fmt.id);
}

...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p11.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p11.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
function rectMask(g,r){app.activeDocument=doc;doc.activeLayer=g;doc.selection.select([[r[0],r[1]],[r[2],r[1]],[r[2],r[3]],[r[0],r[3]]]);var d=new ActionDescriptor(),ref=new ActionReference();d.putClass(charIDToTypeID('Nw  '),charIDToTypeID('Chnl'));ref.putEnumerated(charIDToTypeID('Chnl'),charIDToTypeID('Chnl'),charIDToTypeID('Msk '));d.putReference(charIDToTypeID('At  '),ref);d.putEnumerated(charIDToTypeID('Usng'),charIDToTypeID('UsrM'),charIDToTypeID('RvlS'));executeAction(charIDToTypeID('Mk  '),d,DialogModes.NO);doc.selection.deselect();}
...
try{
 ...
 for(var oi=0;oi<spec.offers.length;oi++){
  ...
  var bk=dup(spec.fixed.background,bgO,'KV'),bb=box(bk);var bgS=s;var bgDX=dx;var horizon=600*s+dy;bgS=Math.max(bgS,bgDX/(-bb[0]),(fmt.w-bgDX)/bb[2],horizon/(600-bb[1]),(fmt.h-horizon)/(bb[3]-600));affine(bk,bgS,bgDX,horizon-600*bgS);

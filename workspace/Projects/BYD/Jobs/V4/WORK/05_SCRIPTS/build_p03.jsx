...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p03.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p03.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
function dup(id,parent,name){app.activeDocument=src;var l=find(src,id);if(!l)throw Error('MISSING '+id);msg("DUP_START|"+id);var cp=l.duplicate(doc);msg("DUP_COPIED|"+id);app.activeDocument=doc;cp=doc.activeLayer;msg('DUP_DEST|'+cp.name+'|parent='+cp.parent.name+'|target='+parent.name);if(parent){cp.move(parent,ElementPlacement.INSIDE);msg('DUP_MOVED|'+id);}if(name)cp.name=name;cp.visible=true;return cp;}
...
try{
 ...
 for(var oi=0;oi<states.length;oi++){for(var j=0;j<states.length;j++)for(var k=0;k<states[j].length;k++)states[j][k].visible=(j===oi);savepng('p03_byd_'+spec.offers[oi].id+'_'+fmt.id);}
 var psdf=new File(BASE+'WORK/01_TEMPLATES/P03_TRIAL_'+fmt.id+'.psd');if(psdf.exists)throw Error('REFUSE_OVERWRITE');var po=new PhotoshopSaveOptions();po.layers=true;po.embedColorProfile=true;doc.saveAs(psdf,po,true,Extension.LOWERCASE);

...
/*
  ...
  Prepared for production_spec_r01.json. Static review required before execution.
...
var sf = new File(BASE + 'WORK/00_MATRIZ/production_spec_r01.json');
if (!sf.exists) throw Error('MISSING_SPEC');
sf.encoding = 'UTF8'; sf.open('r'); var spec = eval('(' + sf.read() + ')'); sf.close();
log = new File(BASE + 'WORK/06_LOGS/build_production_r01.log');
...
function dupSource(id, parentId, name, anchorId, anchorName) { var l = sourceLayer(id), anchorIsRoot = anchorId && sameId(l.id, anchorId), sourceAnchor = anchorId ? (anchorIsRoot ? l : find(l, anchorId)) : l; if (!sourceAnchor) throw Error('SOURCE_ANCHOR_NOT_FOUND|' + id + '|' + anchorId); var sourceBox = box(sourceAnchor), sourceAnchorName = anchorName || sourceAnchor.name; msg('DUP_SOURCE_START|' + id + '|target=' + parentId); l.duplicate(doc); app.activeDocument = doc; var cpId = doc.activeLayer.id; moveInto(cpId, parentId); var cp = fresh(cpId); unlinkTree(cp); var destAnchor = anchorId ? (anchorIsRoot ? cp : byName(cp, cleanName(sourceAnchorName))) : cp; if (!destAnchor) throw Error('DEST_ANCHOR_NOT_FOUND|' + id + '|' + sourceAnchorName); var destBox = box(destAnchor); cp.translate(sourceBox[0] - destBox[0], sourceBox[1] - destBox[1]); cp = fresh(cpId); if (name) cp.name = name; cp.visible = true; msg('DUP_SOURCE_OK|' + id + '|copy=' + cpId + '|target=' + parentId); return cpId; }
...
function sceneMask(groupId, z) { app.activeDocument = doc; doc.activeLayer = fresh(groupId); var x = z[0] - Math.min(50, doc.width.as('px') * .025), y = -100, r = z[0] + z[2] + Math.min(50, doc.width.as('px') * .025), b = doc.height.as('px') + 100; doc.selection.select([[x, y], [r, y], [r, b], [x, b]]); doc.selection.feather(Math.min(30, doc.width.as('px') * .012)); var d = new ActionDescriptor(), ref = new ActionReference(); d.putClass(charIDToTypeID('Nw  '), charIDToTypeID('Chnl')); ref.putEnumerated(charIDToTypeID('Chnl'), charIDToTypeID('Chnl'), charIDToTypeID('Msk ')); d.putReference(charIDToTypeID('At  '), ref); d.putEnumerated(charIDToTypeID('Usng'), charIDToTypeID('UsrM'), charIDToTypeID('RvlS')); executeAction(charIDToTypeID('Mk  '), d, DialogModes.NO); doc.selection.deselect(); }
...
function extraZone(extra, fmt, z) { if (extra.zones && extra.zones[fmt.id]) return extra.zones[fmt.id]; if (extra.zone) return extra.zone; if (fmt.family === 'micro' && extra.role === 'badges') return [8, 23, 77, 12]; if (fmt.family === 'micro' && extra.role === 'instal') return [8, 38, 77, 17]; return z[extra.role] || null; }
...
function verifyZone(label, id, z, tol) { var l = fresh(id), b = box(l), rr = rect(z), visible = l.visible; msg('GEOM|' + currentFmt + '|' + currentOffer + '|' + label + '|bounds=' + b.join(',') + '|zone=' + z.join(',') + '|visible=' + visible); if (visible && (b[0] < rr[0] - tol || b[1] < rr[1] - tol || b[2] > rr[2] + tol || b[3] > rr[3] + tol)) throw Error('GEOM_OUTSIDE_ZONE|' + currentFmt + '|' + currentOffer + '|' + label); }
...
try {
 ...
 preflightOutputs();
 msg('SCRIPT_PREPARED|build_production_r01.jsx|derived=build_p12|patches=p16,p18|review_required=true');
 ...
 for (var fi = 0; fi < spec.formats.length; fi++) {
  ...
  for (var oi = 0; oi < spec.offers.length; oi++) {
   ...
   var vehicleId;
   if (o.car_components && o.car_components.length) { vehicleId = makeGroup(vgId, 'CENA ORIGINAL'); for (var cc = 0; cc < o.car_components.length; cc++) dupSource(o.car_components[cc], vehicleId, null); msg('CAR_COMPONENTS|' + fmt.id + '|' + o.id + '|ids=' + o.car_components.join(',')); }
   ...
   sceneMask(vgId, z.car);
   var titleId = dupSource(o.title, ogId, 'MODELO'); fit(titleId, z.title, fmt.align);
   var priceZone = fmt.family === 'micro' ? (o.micro_dual ? [95, 19, 145, 22] : [95, 19, 145, 25]) : z.price;
   ...
   var carAnchor = byName(fresh(vehicleId), cleanName(o.anchor_name)); if (!carAnchor) throw Error('CAR_ANCHOR_NOT_FOUND|' + o.id); verifyZone('car_anchor', carAnchor.id, z.car, 2);
 ...
 if (pngCount !== 140) throw Error('PNG_COUNT_MISMATCH|' + pngCount);
 msg('OK|completed|formats=7|offers=20|pngs=' + pngCount + '|elapsed_ms=' + (new Date().getTime() - RUN_START));
} catch (e) {

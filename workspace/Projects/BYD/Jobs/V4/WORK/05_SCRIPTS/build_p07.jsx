...
var sf=new File(BASE+'WORK/00_MATRIZ/pilot_spec_p07.json');sf.open('r');var spec=eval('('+sf.read()+')');sf.close();
var log=new File(BASE+'WORK/06_LOGS/build_p07.log');if(log.exists)throw Error('REFUSE_OVERWRITE');log.open('w');
...
function dup(id,parent,name){app.activeDocument=doc;var targetId=parent?parent.id:null;msg('TARGET|'+targetId+'|'+parent.name);app.activeDocument=src;var l=find(src,id);if(!l)throw Error('MISSING '+id);var sourceBox=box(l);msg('DUP_START|'+id);l.duplicate(doc);app.activeDocument=doc;var cp=doc.activeLayer;var destination=find(doc,targetId);msg('DUP_DEST|'+cp.name+'|id='+cp.id+'|target='+destination.name+'|id='+destination.id);if(destination){var marker=destination.artLayers.add();marker.name='TEMP_INSERT';cp.move(marker,ElementPlacement.PLACEBEFORE);marker.remove();msg('DUP_MOVED|'+id);}var destBox=box(cp);cp.translate(sourceBox[0]-destBox[0],sourceBox[1]-destBox[1]);if(name)cp.name=name;cp.visible=true;return cp;}
...
try{
 ...
 for(var oi=0;oi<states.length;oi++){for(var j=0;j<states.length;j++)for(var k=0;k<states[j].length;k++)states[j][k].visible=(j===oi);savepng('p07_byd_'+spec.offers[oi].id+'_'+fmt.id);}
 var psdf=new File(BASE+'WORK/01_TEMPLATES/P07_TRIAL_'+fmt.id+'.psd');if(psdf.exists)throw Error('REFUSE_OVERWRITE');var po=new PhotoshopSaveOptions();po.layers=true;po.embedColorProfile=true;doc.saveAs(psdf,po,true,Extension.LOWERCASE);

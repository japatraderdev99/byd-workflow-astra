...
b=ROOT/'Projects/BYD/Jobs/V4';s=json.loads((b/'WORK/00_MATRIZ/production_spec_r01.json').read_text());out=b/'WORK/04_QA/REVISAO_PRODUCAO_R01.html'
...
parts.append('</nav>')
for f in s['formats']:
 parts.append(f'<section id="{f["id"]}" class="{f["family"]}"><h2>{f["id"]} · <a href="../01_TEMPLATES/2026-09-19-r01/BYD_TPL_{f["id"]}.psd">PSD editável</a></h2>')
 for o in s['offers']:
  src=f'../03_STAGING/2026-09-19-r01/byd_{o["id"]}_{f["id"]}.png';parts.append(f'<article data-offer="{o["id"]}"><figcaption>{o["id"]} · {f["id"]}</figcaption><a href="{src}"><img loading="lazy" src="{src}" alt="{o["id"]} {f["id"]}"></a></article>')

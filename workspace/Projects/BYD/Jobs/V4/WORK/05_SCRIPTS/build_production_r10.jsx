...
/*
  ...
  Prepared for production_spec_r10.json. Static review required before execution.
  ...
  R05 CHANGELOG
  Performance-only revision. Preindex the immutable source once and cache native
  destination objects by layer ID per document. Artwork geometry is unchanged.

  R07 CHANGELOG
  QA-only correction for cropped detail extras. Validate the exact mask rectangle
  against its declared zone and log raw unmasked bounds without rejecting them.
  Artwork geometry and the applied crop mask are unchanged.
*/
(function () {
...
var sourceCache = {}, sourcePathCache = {}, sourceIndexCount = 0, docCache = {};

var sf = new File(BASE + 'WORK/00_MATRIZ/production_spec_r10.json');
if (!sf.exists) throw Error('MISSING_SPEC');
sf.encoding = 'UTF8'; sf.open('r'); var spec = eval('(' + sf.read() + ')'); sf.close();
log = new File(BASE + 'WORK/06_LOGS/build_production_r10.log');
...
function rememberDoc(l) { if (l) docCache[String(l.id)] = l; return l; }
function forgetDoc(id) { delete docCache[String(id)]; }
function fresh(id) { var key = String(id), l = docCache[key]; if (l) { try { if (sameId(l.id, id)) return l; } catch (cachedError) {} forgetDoc(id); } l = find(doc, id); if (!l) throw Error('DEST_ID_NOT_FOUND|' + id); return rememberDoc(l); }
function makeGroup(parentId, n) { var p = parentId === null ? doc : fresh(parentId); var g = p.layerSets.add(); g.name = n; rememberDoc(g); return g.id; }
function indexSource(g, path) { if (!g || !g.layers) return; for (var i = 0; i < g.layers.length; i++) { var l = g.layers[i], p = path.concat([i]); sourceCache[String(l.id)] = l; sourcePathCache[String(l.id)] = p; sourceIndexCount++; if (l.typename === 'LayerSet') indexSource(l, p); } }
function sourceRelativePath(rootId, anchorId) { var rootPath = sourcePathCache[String(rootId)], anchorPath = sourcePathCache[String(anchorId)]; if (!rootPath || !anchorPath || anchorPath.length < rootPath.length) return null; for (var i = 0; i < rootPath.length; i++) if (rootPath[i] !== anchorPath[i]) return null; return anchorPath.slice(rootPath.length); }
function unlinkTree(g) { try { g.unlink(); } catch (e0) {} if (g.typename !== 'LayerSet') return; var children = [], i; for (i = 0; i < g.layers.length; i++) children.push(g.layers[i]); for (i = 0; i < children.length; i++) unlinkTree(children[i]); }
function moveInto(movingId, targetId) { app.activeDocument = doc; var dst = fresh(targetId); if (dst.typename !== 'LayerSet') throw Error('MOVE_TARGET_NOT_GROUP|' + targetId); var marker = dst.artLayers.add(), markerId = marker.id, moving = fresh(movingId); marker.name = '__MK_MOVE_ANCHOR__'; rememberDoc(marker); try { moving.move(marker, ElementPlacement.PLACEBEFORE); } finally { marker.remove(); forgetDoc(markerId); } if (!sameId(moving.parent.id, targetId)) throw Error('MOVE_VERIFY_FAILED|' + movingId + '|' + targetId); return moving; }
function moveManyInto(movingIds, targetId) { app.activeDocument = doc; var dst = fresh(targetId), marker = dst.artLayers.add(), markerId = marker.id; marker.name = '__MK_MOVE_ANCHOR__'; rememberDoc(marker); try { for (var i = 0; i < movingIds.length; i++) fresh(movingIds[i]).move(marker, ElementPlacement.PLACEBEFORE); } finally { marker.remove(); forgetDoc(markerId); } for (var j = 0; j < movingIds.length; j++) if (!sameId(fresh(movingIds[j]).parent.id, targetId)) throw Error('MOVE_VERIFY_FAILED|' + movingIds[j] + '|' + targetId); }
function sourceLayer(id) { app.activeDocument = src; var l = sourceCache[String(id)]; if (!l) throw Error('SOURCE_ID_NOT_FOUND|' + id); return l; }
function dupSource(id, parentId, name, anchorId, anchorName) { var l = sourceLayer(id), sourceRootName = l.name, anchorIsRoot = anchorId && sameId(l.id, anchorId), anchorPath = anchorId ? sourceRelativePath(id, anchorId) : [], sourceAnchor = anchorId ? sourceLayer(anchorId) : l; if (!sourceAnchor) throw Error('SOURCE_ANCHOR_NOT_FOUND|' + id + '|' + anchorId); if (anchorId && anchorPath === null) throw Error('SOURCE_ANCHOR_PATH_NOT_FOUND|' + id + '|' + anchorId); var sourceBox = box(sourceAnchor), sourceAnchorName = anchorName || sourceAnchor.name, sourceAnchorVisible = sourceAnchor.visible; msg('DUP_SOURCE_START|' + id + '|target=' + parentId + '|anchor_path=' + anchorPath.join('.')); l.duplicate(doc); app.activeDocument = doc; var cp = rememberDoc(doc.activeLayer), cpId = cp.id; moveInto(cpId, parentId); cp = fresh(cpId); unlinkTree(cp); var destAnchor = anchorId ? (anchorIsRoot ? cp : byIndexPath(cp, anchorPath)) : cp; if (!destAnchor && anchorId) { destAnchor = byName(cp, cleanName(sourceAnchorName)); msg('ANCHOR_PATH_FALLBACK_NAME|' + id + '|' + sourceAnchorName); } if (!destAnchor) throw Error('DEST_ANCHOR_NOT_FOUND|' + id + '|' + sourceAnchorName); rememberDoc(destAnchor); if (anchorId && anchorName) { destAnchor.name = anchorName; destAnchor.visible = sourceAnchorVisible; } var destBox = box(destAnchor); cp.translate(sourceBox[0] - destBox[0], sourceBox[1] - destBox[1]); cp = fresh(cpId); if (name) cp.name = name; else if (cp.typename === 'ArtLayer') cp.name = sourceRootName; cp.visible = true; msg('DUP_SOURCE_OK|' + id + '|copy=' + cpId + '|target=' + parentId + '|anchor=' + cleanName(destAnchor.name)); return cpId; }
function dupLocal(id, parentId, name) { app.activeDocument = doc; var base = fresh(id), cp = rememberDoc(base.duplicate()), cpId = cp.id; moveInto(cpId, parentId); cp = fresh(cpId); unlinkTree(cp); if (name) cp.name = name; cp.visible = true; return cpId; }
...
function fillRectLayer(parentId, name, r, hex) { var p = fresh(parentId), l = p.artLayers.add(); l.name = name; rememberDoc(l); doc.activeLayer = l; var color = new SolidColor(); color.rgb.hexValue = hex; doc.selection.select([[r[0], r[1]], [r[2], r[1]], [r[2], r[3]], [r[0], r[3]]]); doc.selection.fill(color, ColorBlendMode.NORMAL, 100, false); doc.selection.deselect(); return l.id; }
function fitNamed(rootId, n, z, align) { var l = byName(fresh(rootId), n); if (!l) throw Error('NAMED_LAYER_NOT_FOUND|' + n); rememberDoc(l); return fit(l.id, z, align); }
function applyMicroFooter(fixId, footerId) { var footer = fresh(footerId); footer.visible = true; var bandId = fillRectLayer(fixId, 'FUNDO FAIXA SELOS', [0, 57, 360, 80], '071D31'); fresh(bandId).move(fresh(footerId), ElementPlacement.PLACEAFTER); fitNamed(footerId, 'SELOS GARANTIA E RECOMPRA', [7, 61, 96, 15], 'left'); fitNamed(footerId, 'brasil', [108, 60, 36, 17], 'center'); fitNamed(footerId, 'Clientes Satisfeitos Negativo', [150, 60, 19, 17], 'center'); var ib = byName(fresh(footerId), 'IBAMA'); if (!ib) throw Error('MISSING_IBAMA'); rememberDoc(ib); var icon = byName(ib, 'Vector Smart Object copy'); if (!icon) throw Error('MISSING_IBAMA_ICON'); rememberDoc(icon); fit(icon.id, [176, 61, 13, 16], 'center'); var educational = null; for (var i = 0; i < ib.artLayers.length; i++) if (ib.artLayers[i].kind === LayerKind.TEXT) educational = ib.artLayers[i]; if (!educational) throw Error('MISSING_TRAFFIC_MESSAGE'); rememberDoc(educational); educational.textItem.kind = TextType.POINTTEXT; fit(educational.id, [194, 66, 159, 9], 'left'); msg('MICRO_FOOTER_OK|text=' + educational.textItem.contents.replace(/[\r\n]+/g, ' ')); }
...
function verifyCroppedDetail(label, id, z, maskRect) { var l = fresh(id), raw = box(l), expected = rect(z), visible = l.visible, tol = .01; msg('GEOM_CROPPED|' + currentFmt + '|' + currentOffer + '|' + label + '|raw_bounds=' + raw.join(',') + '|declared_mask=' + maskRect.join(',') + '|zone=' + z.join(',') + '|visible=' + visible); if (!visible) return; for (var i = 0; i < 4; i++) if (Math.abs(maskRect[i] - expected[i]) > tol) throw Error('CROP_MASK_ZONE_MISMATCH|' + currentFmt + '|' + currentOffer + '|' + label); if (maskRect[0] < 0 || maskRect[1] < 0 || maskRect[2] > doc.width.as('px') || maskRect[3] > doc.height.as('px') || maskRect[2] <= maskRect[0] || maskRect[3] <= maskRect[1]) throw Error('CROP_MASK_OUTSIDE_CANVAS|' + currentFmt + '|' + currentOffer + '|' + label); if (raw[2] <= maskRect[0] || raw[0] >= maskRect[2] || raw[3] <= maskRect[1] || raw[1] >= maskRect[3]) throw Error('CROP_CONTENT_MISSES_MASK|' + currentFmt + '|' + currentOffer + '|' + label); }
...
try {
 ...
 if (!spec.formats || spec.formats.length !== 6) throw Error('EXPECTED_6_FORMATS');
 ...
 preflightOutputs();
 var expectedPngCount = spec.formats.length * spec.offers.length;
 msg('SCRIPT_PREPARED|build_production_r10.jsx|derived=build_production_r05|change=cropped_detail_qa_only|expected_pngs=' + expectedPngCount + '|review_required=true');
 app.displayDialogs = DialogModes.NO; app.preferences.rulerUnits = Units.PIXELS;
 src = app.open(new File(BASE + spec.source));
 sourceCache = {}; sourcePathCache = {}; sourceIndexCount = 0; indexSource(src, []); msg('SOURCE_INDEXED|layers=' + sourceIndexCount);
 ...
 for (var fi = 0; fi < spec.formats.length; fi++) {
  ...
  docCache = {}; doc = app.documents.add(fmt.w, fmt.h, 72, 'BYD_TPL_' + fmt.id, NewDocumentMode.RGB, DocumentFill.WHITE, 1, BitsPerChannelType.EIGHT, 'sRGB IEC61966-2.1');
  ...
  var place = new ActionDescriptor(); place.putPath(charIDToTypeID('null'), new File(BASE + spec.logo)); place.putEnumerated(charIDToTypeID('FTcs'), charIDToTypeID('QCSt'), charIDToTypeID('Qcsa')); executeAction(charIDToTypeID('Plc '), place, DialogModes.NO); var logo = rememberDoc(doc.activeLayer), logoId = logo.id; moveInto(logoId, fixId); unlinkTree(fresh(logoId)); fresh(logoId).name = 'LOGO'; fit(logoId, z.logo, 'center');
  ...
  for (var oi = 0; oi < spec.offers.length; oi++) {
   ...
   var carAnchor = byName(fresh(vehicleId), cleanName(o.anchor_name)); if (!carAnchor) throw Error('CAR_ANCHOR_NOT_FOUND|' + o.id); rememberDoc(carAnchor); verifyZone('car_anchor', carAnchor.id, z.car, 2, true);
   if (carTagId) verifyZone('car_tag', carTagId, carTagZ, 2);
   if (z.legal) { verifyZone('legal', legalLayerId, z.legal, Math.max(4, Math.ceil(fmt.legal_font * .5))); verifyLegal(legalLayerId, z.legal); }
   for (var er = 0; er < extraRecords.length; er++) if (extraRecords[er].zone) { if (extraRecords[er].cropped && extraRecords[er].role === 'detail') verifyCroppedDetail(extraRecords[er].label, extraRecords[er].id, extraRecords[er].zone, extraRecords[er].maskRect); else verifyZone(extraRecords[er].label, extraRecords[er].id, extraRecords[er].zone, 2); }
  ...
  var cache = fresh(bgCacheId); cache.remove(); forgetDoc(bgCacheId); fresh(guidesId).visible = false;
  ...
  doc.close(SaveOptions.DONOTSAVECHANGES); doc = null; docCache = {};
 }
 if (pngCount !== expectedPngCount) throw Error('PNG_COUNT_MISMATCH|' + pngCount + '|expected=' + expectedPngCount);
 msg('OK|completed|formats=' + spec.formats.length + '|offers=' + spec.offers.length + '|pngs=' + pngCount + '|elapsed_ms=' + (new Date().getTime() - RUN_START));
} catch (e) {

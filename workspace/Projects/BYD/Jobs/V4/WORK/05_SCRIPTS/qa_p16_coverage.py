...
for f in sorted((base/a.psd_dir).glob(a.revision.upper()+'_TRIAL_*.psd')):
 ...
 visibility_checks={'new_hidden':all(not l.visible for l in layers if l.name.lower()=='new')}
 if psd.size==(360,80):
  by_name={l.name.replace('¬∑','·'):l for l in layers}
  for offer in ('yuan-plus-awd-26-27','dolphin-mini-5l-gs'):
   node=by_name.get('COND · HEADLINE · '+offer);visibility_checks['headline_hidden_'+offer]=node is not None and not node.visible
  node=by_name.get('COND · BENEFICIO · vd-atto-2');visibility_checks['benefit_hidden_vd']=node is not None and not node.visible
 needed=['#GUIAS','FIXO','OFERTA','CONDICIONAIS','LEGAL','VEICULO','BG']
 psds.append(dict(file=str(f.relative_to(base)),size=list(psd.size),sha256=sha(f),bytes=f.stat().st_size,top_groups=top,required_groups_present=all(n in top for n in needed),text_layers=sum(l.kind=='type' for l in layers),smart_objects=sum(l.kind=='smartobject' for l in layers),visibility_checks=visibility_checks,status='DRAFT_3_STATES_NOT_CANONICAL'))
expected_formats={'1920x276','1920x1080','1920x1125','1080x1080','1080x1920','1109x1973','360x80'}
expected_offers={'yuan-plus-awd-26-27','dolphin-mini-5l-gs','vd-atto-2'}
actual={(x['offer_id'],str(x['size'][0])+'x'+str(x['size'][1])) for x in rows}
coverage_ok=actual=={(o,f) for o in expected_offers for f in expected_formats} and len(psds)==7
report=dict(coverage_ok=coverage_ok,at=datetime.now(timezone.utc).isoformat(),elapsed_seconds=round(time.monotonic()-start,3),revision=a.revision,png_count=len(rows),png_pass=sum(x['technical_pass'] for x in rows),psd_count=len(psds),pngs=rows,psds=psds,approval=False)
...
if not coverage_ok or report['png_pass']!=21 or not all(x['required_groups_present'] and all(x['visibility_checks'].values()) for x in psds):raise SystemExit(1)
